...
Page({
	...
	_loadDetail: async function () {
		...
		let carousel = await cloudHelper.callCloudData('admin/carousel_detail', params, opt);
		if (!carousel) {
			this.setData({
				isLoad: null
			})
		...
		let imgList = carousel.CAROUSEL_PIC ? [carousel.CAROUSEL_PIC] : [];

		this.setData({
			isLoad: true,
			imgList: imgList,
			formPic: carousel.CAROUSEL_PIC,
			formUrl: carousel.CAROUSEL_URL || '',
			formOrder: carousel.CAROUSEL_ORDER
		});
	},
	...
	bindFormSubmit: async function () {
		...
		try {
			...
			await cloudHelper.callCloudSumbit('admin/carousel_edit', params);
