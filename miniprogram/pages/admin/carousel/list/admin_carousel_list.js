...
Page({
	...
	bindSortTap: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/carousel_sort', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'CAROUSEL_ORDER', sort);
				this.setData({
	...
	_del: async function (id, that) {
		...
		let callback = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('admin/carousel_del', params, opts).then(res => {
	...
	_setStatus: async function (id, status, that) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/carousel_status', params).then(res => {
				pageHelper.modifyListNode(id, that.data.dataList.list, 'CAROUSEL_STATUS', status, '_id');
				that.setData({
					dataList: that.data.dataList
				});
				// 清除前端缓存
